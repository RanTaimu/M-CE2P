...
def get_data(data_root, set_):
    ...
    ann_root  = data_root + set_ + '/parsing_annos/'

    ann_dict  = obtain_ann_dict(img_root, ann_root)

    flist = [
        line.strip() for line in open(list_root + set_list_add).readlines()
    ]
    ...
    for add in tqdm(flist, desc='Loading %s ..' % (set_list_add)):
        ...
        dat['height'] = im_sz[0]
        dat['bboxes'] = []
        # For every person of 'add', get the bounding box.
